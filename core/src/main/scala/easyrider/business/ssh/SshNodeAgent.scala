...
import easyrider.Configuration.{ConfigurationDeploymentComplete, DeployConfiguration}
...
class SshNodeAgent(eventBus: ActorRef, easyRiderUrl: URL, sshSessionFactory: (NodeConfiguration) => Props) extends Actor with SshNodeDirectoryLayout with ActorLogging {
  ...
  def configured(configuration: NodeConfiguration, sshSession: ActorRef) = LoggingReceive {
    ...
    case DeployConfiguration(commandDetails, containerId, effectiveConfiguration) =>
      log.info("Deploying configuration {} to {}", effectiveConfiguration, containerId)
      val configurationFileContent = "# Auto-generated by EasyRider\n" + effectiveConfiguration.entries.map {
        case (key, value) => s"$key=$value"
      }.mkString("\n")
      sshSession ? SftpUpdateFile(CommandDetails(CommandId.generate(), TraceMode()), configuration.id, containerDir(containerId), "config.properties", ByteString(configurationFileContent)) onSuccess {
        case _: SftpUpdateFileSuccess => eventBus ! ConfigurationDeploymentComplete(EventDetails(EventId.generate(), containerId.eventKey, Seq(commandDetails.commandId)), containerId, effectiveConfiguration)
      }
  }
