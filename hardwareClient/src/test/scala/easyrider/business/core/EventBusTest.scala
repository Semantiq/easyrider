...
class EventBusTest() extends TestKit(ActorSystem()) with FlatSpecLike with Matchers {
  ...
  it should "handle removal events" in {
    ...
    client.send(bus, NodeUpdatedEvent(EventDetails(EventId("1"), EventKey("node0"), Seq(CommandId("1")))))
    client.send(bus, NodeUpdatedEvent(EventDetails(EventId("2"), EventKey("node0"), Seq(CommandId("2")), removal = true)))
  ...
  val dummyEvent = NodeUpdatedEvent(EventDetails(EventId("1"), EventKey(), Seq()))
}
